...
- `action_proposed`: agent proposal (pre-validation)
...
- `action_proposed`: `action`, `rationale`, `alternatives`
...
- `action_proposed` and `observation_built` are emitted only when `logging.observation_events` is true.
